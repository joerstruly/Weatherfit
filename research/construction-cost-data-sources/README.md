...
- `appendix/` — twenty-four underlying research reports: seven topical passes, five verification passes, three commercial-segment passes, five feasibility passes, and four blind-spot passes (non-residential project cost and permit valuation; building efficiency, unit size and typology; GSA price lists, elevators and state assessor manuals; forward escalation, impact fees and A/E fee curves).
...
### 5.12 Forward escalation assumptions (appendix 24, `X` rows)
The under-searched vein turned out to be official state capital-budget assumptions. Maryland DBM publishes a dated multi-year forward curve and revises it visibly — 10%/7.5%/5% in spring 2025 became 4.5%/5.0%/3.5% that autumn. California DOF Budget Letter 26-03 supplies both a rate ceiling of 0.42% per month and, more usefully, a citable public definition of the construction midpoint. San Francisco's AICCIE is the only official sub-national forward curve in the country. Washington CPARB and Kansas OFPM add two more. Federal deflator paths from USACE CWCCIS and the DoD Green Book bound the range below. **Metro resolution remains impossible**: BLS PPI 236500 stops at four Census regions with data only from February 2014, and only two state DOTs attempt to forecast their own index at all.

### 5.13 State assessor unit-in-place cost manuals (appendix 23, `U` rows)
The strongest single find of the blind-spot round. **Michigan's Assessors Manual Volume II publishes Marshall & Swift/Boeckh commercial cost data free as public PDFs**, covering elevators per stop, sprinklers per square foot of sprinklered area, CMU walls, foundations and floor construction, with a 2014 edition still online for a time series. Iowa's Analyzed Unit Cost Schedule is an independent second opinion with unusually transparent method — sprinklers costed at one head per 110 square feet, a 100,000 square foot building basis, plus or minus 20% on exterior walls. Between them they close five catalog gaps at once. Three caveats travel with them: the costs are replacement-cost-new for assessment, they carry an embedded overhead-and-profit loading that must be backed out, and Michigan's underlying tables are Marshall & Swift copyright, so **a public PDF is not automatically redistributable**.

### 5.14 Gross SF per unit and unit size (appendix 22, `E` rows)
Assessor and benchmarking datasets carry gross building area and residential unit count on the same record, which is what the model actually needs: NYC PLUTO across roughly 860,000 tax lots with `BldgArea`, `UnitsRes`, `NumFloors` and `BldgClass`; Maryland's statewide CAMA on Socrata; Maricopa County's Apartment Master; and the Local Law 84 family of energy-benchmarking disclosures in a dozen large cities. For the net side, UDR's 10-K reports average home size by market with a ten-year back-series, and **Census does publish multifamily unit square footage** through the Survey of Construction, which an earlier pass had wrongly treated as unknown. Academic anchors for the height cost curve: Eriksen and Orlando across the 50 largest cities, and Abt's HUD study across 2,500-plus LIHTC projects.

### 5.15 Non-residential project cost and permit valuation (appendix 21, `G` rows)
MSBA's cost data and GSA's prospectus library are the Tier A validation set — real owner budgets with gross square footage attached. Permit valuation is a trap dressed as a dataset: where the ICC Building Valuation Data table is adopted, declared valuation *is* gross area times a published rate, so regressing cost on area recovers the fee table rather than the market. Only Austin, Boston and New York were confirmed to publish value, square footage and use type together. **Census discontinued non-residential permit collection in 1995**, so the Building Permits Survey cannot validate commercial work at all.
